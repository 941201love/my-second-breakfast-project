...
export default function App() {
  ...
  async function loadMenu(): Promise<void> {
    ...
    void fetch(buildApiUrl("/api/promotions/active"))
      .then(async (promotionsResponse) => {
        if (!promotionsResponse.ok) return;
        const promotionsPayload =
          (await promotionsResponse.json()) as ApiDataResponse<
            ActivePromotion[]
          >;
        const activePromotionByLogicalId = new Map(
        ...
        if (activePromotionByLogicalId.size === 0) return;
          ...
          <section className="grid grid-cols-1 gap-8 lg:grid-cols-[minmax(0,1fr)_360px]">
            <div className="space-y-8 py-6">
              ...
              <section className="rounded-lg border border-base-300 bg-base-200 p-4">
                <div className="mb-3 text-sm font-bold">商品標籤</div>
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  ...
                  <label className="flex cursor-pointer items-center justify-between gap-4 rounded-lg border border-base-300 bg-base-100 p-4">
                    <span>
                      <span className="block font-semibold">新品推出</span>
                      <span className="text-sm opacity-70">
                        手動顯示在前台新品推出區，不再依照 7 天自動判斷。
                      </span>
                    </span>
                    <input
                      className="toggle toggle-primary"
      ...
      {user && (isProfileOpen || isProfilePage) ? (
        <>
          <div
            ...
            aria-hidden="true"
          />
          <section className="customer-profile-modal fixed left-1/2 top-1/2 w-[calc(100%-2rem)] max-w-md -translate-x-1/2 -translate-y-1/2 rounded-lg bg-base-100 shadow-2xl">
            <div className="p-4 border-b border-base-300 flex items-center justify-between">
